/**
 * 清理 issues 桶中“markdown/comment 孤儿图片”的两阶段脚本（默认 dry-run）：
 * 1) 软删阶段：把“超期且未被业务文本/附件关系引用”的记录标记为 inactive
 * 2) 硬删阶段：删除“已 inactive 且再次超期”的文件与数据库记录
 *
 * 用法示例：
 * - 仅预览（默认）：npm --prefix apps/hub-v2/server run uploads:cleanup-issues-orphan
 * - 执行软删：npm --prefix apps/hub-v2/server run uploads:cleanup-issues-orphan -- --apply
 * - 执行软删+硬删：npm --prefix apps/hub-v2/server run uploads:cleanup-issues-orphan -- --apply --hard-delete
 * - 自定义阈值：npm --prefix apps/hub-v2/server run uploads:cleanup-issues-orphan -- --apply --keep-days=14 --hard-delete-days=7 --limit=500
 *
 * 参数说明：
 * - --apply：从 dry-run 切换为真实执行（不传则仅输出统计）
 * - --hard-delete：开启硬删阶段（不传则只做 soft-delete）
 * - --keep-days=N：active issues 候选保留天数（默认 14）
...
function listSoftDeleteCandidates(db: Database.Database, keepDays: number, limit: number | null): UploadRow[] {
  const sql = `
    ...
    FROM uploads u
    WHERE u.bucket = 'issues'
      AND u.status = 'active'
      AND (u.category = 'comment' OR u.category LIKE 'markdown%')
      AND datetime(u.created_at) < datetime('now', '-' || @keepDays || ' day')
      AND NOT EXISTS (SELECT 1 FROM issue_attachments ia WHERE ia.upload_id = u.id)
      AND NOT EXISTS (SELECT 1 FROM issues i WHERE i.description LIKE '%/api/admin/uploads/' || u.id || '/raw%')
      AND NOT EXISTS (SELECT 1 FROM issues i WHERE i.resolution_summary LIKE '%/api/admin/uploads/' || u.id || '/raw%')
      AND NOT EXISTS (SELECT 1 FROM issues i WHERE i.close_remark LIKE '%/api/admin/uploads/' || u.id || '/raw%')
      ...
      AND NOT EXISTS (SELECT 1 FROM issue_logs l WHERE l.meta_json LIKE '%/api/admin/uploads/' || u.id || '/raw%')
