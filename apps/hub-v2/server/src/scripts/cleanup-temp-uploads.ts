...
function listSoftDeleteCandidates(db: Database.Database, keepDays: number, limit: number | null): UploadRow[] {
  const sql = `
    ...
    WHERE u.bucket = 'temp'
      ...
      AND NOT EXISTS (SELECT 1 FROM rd_logs rl WHERE rl.meta_json LIKE '%/api/admin/uploads/' || u.id || '/raw%')
      AND NOT EXISTS (SELECT 1 FROM rd_item_stage_notes rn WHERE rn.description LIKE '%/api/admin/uploads/' || u.id || '/raw%')
      AND NOT EXISTS (SELECT 1 FROM rd_stage_tasks rt WHERE rt.description LIKE '%/api/admin/uploads/' || u.id || '/raw%')
