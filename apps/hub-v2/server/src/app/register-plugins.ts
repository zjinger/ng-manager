    ...
    fp(async (fastify) => {
      ...
      fastify.addHook("onRequest", async (request) => {
        ...
        if (requestPath.startsWith("/api/token/") && bearerToken) {
          ...
          if (verifiedToken) {
            const now = Date.now();
            tokenRateLimiter.enforce(`project:${verifiedToken.tokenId}`, now);
            tokenRateLimiter.cleanup(now);
        ...
        if (requestPath.startsWith("/api/personal/") && bearerToken) {
          ...
          if (verifiedToken) {
            const now = Date.now();
            tokenRateLimiter.enforce(`personal:${verifiedToken.tokenId}`, now);
            tokenRateLimiter.cleanup(now);
            request.requestContext = createRequestContext({
              accountId: verifiedToken.tokenId,
              userId: verifiedToken.ownerUserId,
        ...
        if (payload && accountId !== "anonymous" && !(await fastify.container.authQuery.isAccountActive(accountId))) {
          payload = null;
        }
        const activeAccountId = payload?.accountId ?? "anonymous";
        const permissionCodes =
          payload && activeAccountId !== "anonymous"
            ? await fastify.container.authQuery.listPermissionCodesByAccountId(activeAccountId)
            : [];
