...
export interface ReleaseQueryContract {
  ...
  listRecentPublishedForNotifications(projectIds: string[], limit: number, ctx: RequestContext): Promise<ReleaseEntity[]>;
  listRecentArchivedForNotifications(projectIds: string[], limit: number, ctx: RequestContext): Promise<ReleaseEntity[]>;
}
