...
export class PersonalTokenService implements PersonalTokenCommandContract, PersonalTokenQueryContract {
  ...
  getProjectCapabilities(projectKey: string, ctx: RequestContext): PersonalProjectCapabilitiesResult {
    ...
    const issueCaps = {
      ...
      canUpdate: scopeSet.has("issue:update:write"),
    ...
    const rdCaps = {
      canCreateRdItem: scopeSet.has("rd:create:write"),
      canUpdateProgress: scopeSet.has("rd:progress:write") || scopeSet.has("rd:transition:write"),
      canCreateRdStageTask: scopeSet.has("rd:stage-task:write"),
    ...
    const hasWriteScope =
      ...
      docsCaps.canUpdate ||
      docsCaps.canPublish ||
      rdCaps.canCreateRdItem ||
      rdCaps.canUpdateProgress ||
      rdCaps.canCreateRdStageTask ||
