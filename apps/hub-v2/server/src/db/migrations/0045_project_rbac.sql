...
VALUES
  ...
  ('sperm_project_manage', 'project.manage', '管理项目', 'project', '项目管理', 'project', '项目管理', '用于项目治理管理能力判定。', 15, datetime('now'), datetime('now')),
...
WHERE code IN ('project.create', 'project.manage', 'project.read.all', 'project.manage.all', 'project.archive', 'project.owner.transfer');
...
FROM system_permissions
WHERE code IN ('project.create', 'project.manage');
