...
import { TravelExpenseBasicInfoComponent } from '@app/features/reimbursement/shared/components/travel-expense-basicInfo/travel-expense-basicInfo.component';
import { ExpenseDetailsComponent } from '@app/features/reimbursement/shared/components/expense-details/expense-details.component';
...
export class AddTravelExpense implements OnInit {
  ...
  async saveDraft(): Promise<void> {
    ...
    if (success) {
      ...
      this.modalService.success({
        ...
        nzClosable: false,  
        nzOnOk: () => {
          this.router.navigate(['/reimbursements/mine']);
        },
        nzCancelText: '继续编辑',
        nzOnCancel: () => {
          // 留在当前页面继续编辑
  ...
  async confirmSubmit(action: string): Promise<void> {
    ...
    if (action === 'submit') {
        const success = await this.store.submitApproval(this.submitComment());
        if (success) {
          this.submitComment.set('');
        // 提交成功后跳转到列表页
        this.modalService.success({
          nzTitle: '提交成功',
