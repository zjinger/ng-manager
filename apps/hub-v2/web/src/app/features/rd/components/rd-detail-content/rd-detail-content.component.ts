            ...
            <div class="action-card__buttons">
              @for (action of actionButtons(); track action.key) {
                @if (action.confirm) {
                  ...
                  <button nz-button [nzType]="action.primary ? 'primary' : 'default'" [disabled]="busy()" (click)="actionClick.emit(action.key)">
                    {{ action.label }}
                  </button>
                }
              }
            ...
            @if (allowProgressEdit()) {
              <div class="action-card__progress">
                ...
                <div class="action-card__progress-actions">
                  <button nz-button nzType="default" [disabled]="busy() || !progressDirty()" (click)="resetProgress()">重置</button>
                  <button
                    nz-button
                    nzType="primary"
