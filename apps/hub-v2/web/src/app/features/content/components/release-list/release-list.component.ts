...
import type { ReleaseEntity } from '../../models/content.model';
...
@Component({
        ...
        @for (item of items(); track item.id) {
          <div class="content-row">
            <div class="content-cell content-cell--title">
              ...
              <div class="content-meta">{{ item.notes || '暂无说明' }}</div>
            </div>
            <div class="content-cell">{{ item.channel }}</div>
            <div class="content-cell">{{ item.version }}</div>
            <div class="content-cell">{{ statusLabel(item.status) }}</div>
            <div class="content-cell content-cell--muted">{{ item.publishedAt ? (item.publishedAt | date: 'MM-dd HH:mm') : '未发布' }}</div>
  ...
  styles: [
    `
      ...
      .content-row {
        display: grid;
        grid-template-columns: 2fr 0.9fr 0.8fr 0.8fr 0.9fr 0.9fr;
        gap: 16px;
        align-items: center;
      }
      .content-table__head {
...
export class ReleaseListComponent {
  readonly items = input<ReleaseEntity[]>([]);
  readonly edit = output<ReleaseEntity>();
  readonly publish = output<ReleaseEntity>();
