...
export class ProjectRepoJsonKv implements ProjectRepo {
    constructor(private kv: KvRepo<Project>) { }
    ...
    async create(p: Project): Promise<void> {
        ...
        if (existed) throw new CoreError(CoreErrorCodes.PROJECT_ALREADY_EXISTS, `Project already exists: ${p.id}`);
        await this.kv.set(p.id, p);
    }

    async update(id: string, patch: Partial<Project>): Promise<Project> {
        const cur = await this.kv.get(id);
        if (!cur) throw new CoreError(CoreErrorCodes.PROJECT_NOT_FOUND, `Project not found: ${id}`);
