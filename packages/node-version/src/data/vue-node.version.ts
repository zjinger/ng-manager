...
export function getEnginesByVue(vueVersion: number): { node: string } {
  const config = VUE_NODE_VERSIONS.find(v => v.vueVersion === vueVersion);
  if (!config) {
    throw new CoreError(CoreErrorCodes.INVALID_NAME, `不支持的 Vue 版本: ${vueVersion}`);
  }
  return { node: config.supportedNodeRange };
}

export { VUE_NODE_VERSIONS };
