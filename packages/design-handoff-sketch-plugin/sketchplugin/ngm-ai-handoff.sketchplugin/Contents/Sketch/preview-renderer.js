...
function dataAttrs(node, kind) {
    return (' data-handoff-id="' + escapeHtml(node.handoffId || node.id) + '"' +
...
function renderNode(node, styleMap, assetsByLayerId, output) {
    ...
    if (asset && asset.path) {
        output.push('<img class="ngm-layer ngm-image"' +
        ...
        output.push('<div class="ngm-layer ngm-text"' +
            dataAttrs(node, "text") +
        ...
        if (hasVisualStyle || node.type === "Shape" || node.type === "ShapePath" || node.role === "button" || node.role === "input" || node.role === "card" || node.role === "modal") {
            output.push('<div class="ngm-layer ngm-shape"' +
