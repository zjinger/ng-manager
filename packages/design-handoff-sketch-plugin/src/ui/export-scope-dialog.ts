// @ts-nocheck
// 自定义导出勾选对话框。
// 使用 cocoascript 构建 NSScrollView + checkbox 列表 + 工具按钮。
// 第一版不做复杂树形 UI，仅做平铺勾选。
...
function buildPromptLabel(content, entries) {
  var label = NSTextField.alloc().initWithFrame(
    nsRect(PADDING, WINDOW_HEIGHT - PADDING - 24, WINDOW_WIDTH - PADDING * 2, 24),
  );
...
function buildScrollView(content, entries, window) {
  ...
  var scrollView = NSScrollView.alloc().initWithFrame(
    nsRect(PADDING, scrollBottom, WINDOW_WIDTH - PADDING * 2, scrollHeight),
  );
  ...
  var documentView = NSView.alloc().initWithFrame(
    nsRect(0, 0, WINDOW_WIDTH - PADDING * 2 - 2, documentHeight),
  );
...
function buildBottomButtons(content, window) {
  var confirmButton = NSButton.alloc().initWithFrame(
    nsRect(WINDOW_WIDTH - PADDING - 200 - 8 - 80, PADDING, 80, 28),
  );
  ...
  var cancelButton = NSButton.alloc().initWithFrame(
    nsRect(WINDOW_WIDTH - PADDING - 80, PADDING, 80, 28),
  );
...
// 用户点击窗口左上角 x 关闭按钮时，将操作视作取消并停止模态会话，
// 避免界面已关闭而 runModalForWindow 仍阻塞。
