...
export function buildIndexObject(options: {
  ...
  let orderedPageKeys: string[] = [];
  let artboards: DocumentIndexArtboardDto[] = [];
...
export function generateIndexHtml(indexObject: DocumentIndexDto, modeLabel?: string): string {
  ...
  lines.push("  body { margin:0; font-family:-apple-system, BlinkMacSystemFont, 'PingFang SC', 'Segoe UI', sans-serif; background:" + cssVariable("--bg") + "; color:" + cssVariable("--text") + "; }");
  ...
  lines.push("  .ngm-meta { color:" + cssVariable("--muted") + "; font-size:13px; line-height:1.7; overflow-wrap:anywhere; }");
  lines.push("  .ngm-summary { flex:0 0 auto; display:grid; grid-template-columns:repeat(5, minmax(70px, 1fr)); gap:8px; min-width:420px; }");
  lines.push("  .ngm-summary-item { background:" + cssVariable("--panel") + "; border:1px solid " + cssVariable("--line") + "; border-radius:8px; padding:10px 12px; }");
  lines.push("  .ngm-summary-value { font-size:18px; font-weight:700; line-height:1.1; }");
  lines.push("  .ngm-summary-label { margin-top:4px; color:" + cssVariable("--muted") + "; font-size:12px; }");
  ...
  lines.push("  .ngm-page-count { color:" + cssVariable("--muted") + "; font-weight:500; font-size:12px; }");
  lines.push("  .ngm-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(260px, 1fr)); gap:14px; }");
  lines.push("  .ngm-card { min-width:0; overflow:hidden; background:" + cssVariable("--panel") + "; border:1px solid " + cssVariable("--line") + "; border-radius:8px; }");
  lines.push("  .ngm-thumb { display:flex; align-items:center; justify-content:center; height:180px; background:#edf1f5; border-bottom:1px solid " + cssVariable("--line") + "; }");
  lines.push("  .ngm-thumb img { display:block; max-width:100%; max-height:100%; width:auto; height:auto; }");
  lines.push("  .ngm-thumb-missing { color:" + cssVariable("--muted") + "; font-size:12px; }");
  ...
  lines.push("  .ngm-tag.ok { background:#e8f7ee; color:" + cssVariable("--ok") + "; }");
  lines.push("  .ngm-tag.fail { background:#fdecec; color:" + cssVariable("--fail") + "; }");
  lines.push("  .ngm-links { display:flex; flex-wrap:wrap; gap:8px; margin-top:12px; }");
  lines.push("  .ngm-link { display:inline-flex; align-items:center; height:28px; padding:0 10px; border:1px solid " + cssVariable("--line") + "; border-radius:6px; color:" + cssVariable("--link") + "; text-decoration:none; font-size:12px; background:#fff; }");
  lines.push("  .ngm-link:hover { border-color:#93b4f6; background:#f7faff; }");
  lines.push("  .ngm-reason { margin-top:10px; color:" + cssVariable("--fail") + "; font-size:12px; line-height:1.5; overflow-wrap:anywhere; }");
