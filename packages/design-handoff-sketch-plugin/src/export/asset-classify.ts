...
var SMALL_VECTOR_MAX = 128;

var ICON_RE = /\bicon\b|图标|\bic[-_/]|svg/i;
var LOGO_RE = /\blogo\b|品牌标|商标/i;
...
function isMostlyShapePath(layer) {
...
function classifyAsset(layer) {
  ...
  var type = layer.type || "Unknown";
  var sourceLayerType = type;
  var name = lowerName(layer);
  var frame = getFrame(layer);
  var exportable = hasExportFormats(layer);

  // 规则 1
  if (type === "Slice") {
    return { type: "slice", sourceLayerType: sourceLayerType, exportable: exportable };
  }
  // 规则 2
  if (type === "Image" || type === "Bitmap") {
    return { type: "bitmap", sourceLayerType: sourceLayerType, exportable: exportable };
  }

  // 命名优先（规则 3/4）：logo 优先于 icon，便于 preview 优先用图。
  if (LOGO_RE.test(name)) {
    return { type: "logo", sourceLayerType: sourceLayerType, exportable: exportable };
  }
  if (ICON_RE.test(name)) {
    return { type: "icon", sourceLayerType: sourceLayerType, exportable: exportable };
  }

  // 规则 5：SymbolInstance / SymbolMaster
  if (type === "SymbolInstance" || type === "SymbolMaster") {
    return { type: "symbol", sourceLayerType: sourceLayerType, exportable: exportable };
  }
...
function assetTypeDirectory(type) {
  var map = {
    bitmap: "images",
    image: "images",
    slice: "slices",
